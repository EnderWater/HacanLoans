...
export class App {
    ...
    currentModal;
    openHamburgerMenu;
    constructor(isDebug, clearLocalStorage) {
    ...
    addEventListeners() {
        ...
        playerGrid.addEventListener("click", (event) => {
            const button = event.target
                .closest(".player-card-edit-button");
            if (!button) {
                return;
            }
    ...
    `;
        ...
        modal?.addEventListener("click", event => {
            const click = event;
            if (!modalBox.contains(click.target))
                this.closeModal();
        });
    ...
    openPlayerModal(playerId) {
        ...
        this.currentModal.querySelector("#player-form")
            ...
            const updatedPlayer = {
                name: (this.currentModal?.querySelector("#player-name")).value,
                faction: Number((this.currentModal?.querySelector("#player-faction")).value),
                loanAmount: Number((this.currentModal?.querySelector("#loan-amount")).value),
                loanRound: Number((this.currentModal?.querySelector("#loan-round")).value),
                loanPercentage: Number((this.currentModal?.querySelector("#loan-percentage")).value)
            };
    ...
    openRoundModal() {
        ...
        modal.querySelector("#update-round-button")?.addEventListener("click", () => {
            const round = Number(modal.querySelector("#round-input").value);
    ...
    openPaymentModal() {
        ...
        modal.querySelector("#add-payment")?.addEventListener("click", () => {
            const form = {
                player: Number(modal.querySelector("#player-selection").value),
                payment: Number(modal.querySelector("#payment-amount").value),
                round: Number(modal.querySelector("#payment-round").value),
            };
            if (!form.payment || !form.player || !form.round || Number.isNaN(form.player)
                || form.payment <= 0 || form.round <= 0)
                return;
    ...
    openLoanModal() {
        ...
        modal.querySelector("#add-loan")?.addEventListener("click", () => {
            const form = {
                playerId: Number(modal.querySelector("#player-selection-loan").value),
                loan: Number(modal.querySelector("#loan-amount").value),
                round: Number(modal.querySelector("#loan-round").value),
                percentage: Number(modal.querySelector("#loan-percentage").value),
            };
